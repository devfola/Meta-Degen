...
contract DegenToken is ERC20, Ownable {
    ...
    mapping(address => mapping(bytes32 => GameItem)) public playerItems;
    ...
    function playerRegister(string memory _playerName) external {
        ...
        if (players[msg.sender].player != address(0)) revert YOU_HAVE_REGISTERED();
    ...
    function mint(address _to, uint256 _amount) public onlyOwner {
        if (!players[msg.sender].isRegistered) revert PLAYER_NOT_REGISTERED();
        _mint(_to, _amount);
    }
    ...
    function addGameItem(string calldata _itemName, uint256 _amount) external onlyOwner{
